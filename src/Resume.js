...
class Resume extends React.Component{
    componentDidMount() {
        const jsSource = [
            'function actDragStart(a){var b=document.getElementById("dragTemp"),d=this.cloneNode(!0);d.style.display="none";b.innerHTML="";b.appendChild(d);a.dataTransfer.setDragImage(d,0,0)}',
            ...
            'function actDrop(a){a.preventDefault();var b=document.querySelector(".js-temp");if((a=window.myDragDOM.style.transform.match(/-?[0-9]+/))&&0<parseInt(a)-parseInt(window.myDragDOM.getAttribute("eventTranslate"))){var d=window.myDragDOM.getBoundingClientRect().bottom;document.querySelectorAll(".container-wayne .row-wayne").forEach(function(a){a!==window.myDragDOM&&a.getBoundingClientRect().top<d&&(b=a)});b.parentNode.insertBefore(window.myDragDOM,b.nextSibling)}else d=window.myDragDOM.getBoundingClientRect().top,',
            'document.querySelectorAll(".container-wayne .row-wayne").forEach(function(a){a!==window.myDragDOM&&a.getBoundingClientRect().top<d&&(b=a)}),b.parentNode.insertBefore(window.myDragDOM,b);resetDragStatus()}function actMouseup(){window.myDragDOM&&resetDragStatus()}function actDragend(){if(window.myDragDOM){var a=document.querySelector(".js-temp");a.parentNode.insertBefore(window.myDragDOM,a);resetDragStatus()}}',
            'function resetDragStatus(){window.myDragDOM.classList.remove("action-drag");window.myDragDOM.style.transform=null;window.myDragDOM.style.width=null;window.myDragDOM=null;removeTempRow()}function removeTempRow(){document.querySelectorAll(".js-temp").forEach(function(a){a.parentNode.removeChild(a)})}',
